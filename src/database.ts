  ...
  const db = await openDB(DATABASE_NAME, VERSION, {
    upgrade(db, oldVersion, newVersion, transaction) {
      ...
      if (!db.objectStoreNames.contains(MEAL_PLAN_STORE_NAME)) {
        db.createObjectStore(MEAL_PLAN_STORE_NAME, {
          keyPath: "id",
          autoIncrement: true,
        });
      }
...
// export async function getProducts() {
//   const db = await initDB();
//   const tx = db.transaction("ingredientsStore", "readonly"); // Załóżmy, że składniki są przechowywane w 'ingredientsStore'
//   const store = tx.objectStore("ingredientsStore");
